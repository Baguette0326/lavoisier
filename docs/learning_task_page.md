# CarbonSense Learning Task Page

This page defines what you need to understand well enough to explain CarbonSense to a chemical-engineering recruiter, professor, or research supervisor.

## Project Scope Decision

The first CarbonSense MVP will focus on:
...
6. Why CarbonSense is different from asking a generic LLM to summarize a paper.
...
> CarbonSense starts with post-combustion-style CO2/N2 screening because the first dataset target naturally supports CO2 and N2 adsorption comparisons under controlled simulation conditions.
...
Understand why CarbonSense blocks weak comparisons.
...
> CarbonSense can approve that a value was extracted correctly while still blocking it from ranked comparison if the measurement conditions are not comparable.
...
> The first MVP uses computational records as screening evidence only. CarbonSense does not claim that the top-ranked material is experimentally validated.

### 7. Software Differentiator

Understand the difference between CarbonSense and a generic LLM.
...
> CarbonSense turns AI-assisted reading into a controlled engineering review pipeline. The backend enforces domain rules before records can be ranked.
...
Carbonate polymorphs such as calcite, aragonite, and vaterite matter in CO2 mineralization. This is related to permanent carbon storage or utilization, but it is outside the first CarbonSense post-combustion adsorption MVP.

Key explanation:

> CarbonSense must identify the specific material form, not just the material name. The same formula or MOF family can contain polymorphs, framework isomers, or linker isomers with meaningfully different adsorption performance.
...
- Why CarbonSense focuses on comparability before ranking.
...
Read the CarbonSense source research report and summarize:
...
> I built CarbonSense to help structure and compare carbon-capture MOF screening records. The key challenge is that adsorption data is only meaningful under its operating and simulation conditions. CarbonSense extracts or ingests records, validates required context, blocks non-comparable records from ranking, and exports a reviewable table with provenance and warnings. It does not replace experiments or claim to discover a best material; it makes early-stage screening more transparent and reproducible.
...
7. Explain why CarbonSense is a backend comparability tool, not just an LLM wrapper.

## What Not To Overclaim

Do not say:

- CarbonSense discovers new MOFs.
- CarbonSense proves which material is best.
...
> CarbonSense helps produce a transparent, source-linked, comparison-aware shortlist for human review.
